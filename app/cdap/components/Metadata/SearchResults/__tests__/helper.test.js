...
describe('Metadata search helper', () => {
      ...
      metadata: {
        properties: [
          { scope: 'SYSTEM', name: 'creation-time', value: '1631803810545' },
          {
            ...
            name: 'schema',
            value:
              '{"type":"record","name":"etlSchemaBody","fields":[{"name":"body","type":"logs"}]}',
          },
        ...
        tags: [
          { scope: 'SYSTEM', name: 'logs' },
          { scope: 'USER', name: 'logs' },
        ],
      },
    },
    {
      ...
      metadata: {
        ...
        tags: [
          { scope: 'SYSTEM', name: 'batch' },
          { scope: 'SYSTEM', name: 'explore' },
        ],
      },
    },
  ];
  ...
  const metadataFilters = METADATA_FILTERS.map((name) => ({
    name,
    isActive: true,
    filter: '',
    count: 0,
  ...
  const resultSource = {
    ...
    queryFound: ['Name', 'User properties', 'System properties'],
  ...
  describe("'parseResult' should", () => {
    it('return parsed results and count', () => {
      expect(parseResult('logs', entities, metadataFilters)).toStrictEqual({
