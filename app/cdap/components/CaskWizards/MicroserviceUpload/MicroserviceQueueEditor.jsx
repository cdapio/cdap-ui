...
const mapStateToFieldNameProps = (state, ownProps) => {
  return {
    ...
    sslKeystoreFilePath:
      state.general.queues[ownProps.index].properties['ssl-keystore-file-path'],
    sslKeystorePassword:
      state.general.queues[ownProps.index].properties['ssl-keystore-password'],
    sslKeystoreKeyPassword:
      state.general.queues[ownProps.index].properties[
        'ssl-keystore-key-password'
      ],
    sslKeystoreType:
      state.general.queues[ownProps.index].properties['ssl-keystore-type'],
    ...
    sslTruststoreType:
      state.general.queues[ownProps.index].properties['ssl-truststore-type'],
    ...
    valueSerdes:
      state.general.queues[ownProps.index].properties['value-serdes'],
  };
};
...
const MicroserviceQueueCopy = connect(
...
export default class MicroserviceQueueEditor extends Component {
  ...
  constructor(props) {
    ...
    this.sub = this.MicroserviceQueueStore.subscribe(() => {
      const queues = this.MicroserviceQueueStore.getState().general.queues;
      onChange(queues);
      ...
      return (
        <div
          className={classnames(
            ...
            'microservice-add-first-queue-container'
          )}
      ...
      <Provider store={this.MicroserviceQueueStore} key={uuidV4()}>
        <div className="microservice-queue-editor-container">
          {this.state.queues.map((field, index) => {
            return <MicroserviceQueueCopy index={index} />;
          })}
        </div>
      </Provider>
    );
