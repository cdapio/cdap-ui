...
public class CapabilityManagerTest extends AppFabricTestBase {
  ...
  public void testGetAppsWithCapability() throws Exception {
    ...
    for (String capability : declaredAnnotation.capabilities()) {
      CapabilityApplications appsForCapability = capabilityManager
        .getAppsForCapability(NamespaceId.DEFAULT, capability, null, 0, 10);
      Set<ApplicationId> applicationIds = appsForCapability.getApplicationIds();
    ...
    for (String capability : declaredAnnotation.capabilities()) {
      Set<ApplicationId> applicationIds = capabilityManager
        .getAppsForCapability(NamespaceId.DEFAULT, capability, null, 0, 10).getApplicationIds();
      List<ApplicationDetail> appsReturned = applicationLifecycleService.getAppDetails(applicationIds).values().stream()
        .collect(Collectors.toList());
  ...
  public void testGetAppsForCapabilityPagination() throws Exception {
    ...
    CapabilityApplications appsForCapability = capabilityManager
      .getAppsForCapability(NamespaceId.DEFAULT, capability, null, 0, 1);
    Assert.assertEquals(1, appsForCapability.getApplicationIds().size());
    //next search with pagination
    CapabilityApplications appsForCapabilityNext = capabilityManager
      .getAppsForCapability(NamespaceId.DEFAULT, capability, appsForCapability.getCursor(), 1, 1);
    Assert.assertEquals(1, appsForCapabilityNext.getApplicationIds().size());
    CapabilityApplications appsForCapabilityNext1 = capabilityManager
      .getAppsForCapability(NamespaceId.DEFAULT, capability, appsForCapability.getCursor(), 2, 1);
    Assert.assertEquals(0, appsForCapabilityNext1.getApplicationIds().size());
  }
  ...
  public void testIsApplicationDisabled() throws Exception {
    ...
    boolean applicationDisabled = capabilityManager
      .isApplicationDisabled(NamespaceId.DEFAULT.getNamespace(), appNameWithCapability);
    ...
    //capabilityManager.
    //cConfiguration.set(Constants.AppFabric.ENABLED_CAPABILITY_LIST, String.join(",", capabilities));

    applicationDisabled = capabilityManager
      .isApplicationDisabled(NamespaceId.DEFAULT.getNamespace(), appNameWithCapability);
    Assert.assertEquals(false, applicationDisabled);

    //applications with no acclerators should not be disabled
    boolean applicationDisabled1 = capabilityManager
      .isApplicationDisabled(NamespaceId.DEFAULT.getNamespace(), appNameWithOutCapability);
    Assert.assertEquals(false, applicationDisabled);
