...
public class CapabilityEnableExecutor implements CapabilityActionExecutor {
  ...
  @Inject
  public CapabilityEnableExecutor(CapabilityManager capabilityManager) {
  ...
  public void execute(CapabilityStep capabilityStep) {
    ...
    try {
      boolean hasChanged = CapabilityStatus.ENABLED == capabilityManager.getCapabilityStatus(capability);
      if (hasChanged) {
        //TODO - full steps
        // Update status
        // start app
      } else {
        //TODO - ensure service is running if includes an application
      }
