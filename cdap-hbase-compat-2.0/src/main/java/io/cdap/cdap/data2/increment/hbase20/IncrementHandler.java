 ...
 */
public class IncrementHandler implements RegionObserver, RegionCoprocessor {
  ...
  public void start(CoprocessorEnvironment e) {
    if (e instanceof RegionCoprocessorEnvironment) {
      ...
      this.state = new IncrementHandlerState(n -> env.getConnection().getTable(n), env.getConfiguration(),
                                             tableDesc.getValue(Constants.Dataset.TABLE_PREFIX));
  ...
  private Map<byte[], byte[]> convertFamilyValues(Map<Bytes, Bytes> writableValues) {
    Map<byte[], byte[]> converted = Maps.newTreeMap(Bytes.BYTES_COMPARATOR);
    for (Map.Entry<Bytes, Bytes> e : writableValues.entrySet()) {
  ...
  @Override
  public void preGetOp(ObserverContext<RegionCoprocessorEnvironment> ctx,
                       Get get, List<Cell> results) throws IOException {
    Scan scan = new Scan(get);
    scan.readAllVersions();
    scan.setFilter(Filters.combine(new IncrementFilter(), scan.getFilter()));
    try (RegionScanner scanner = new IncrementSummingScanner(region, scan.getBatch(),
                                                             region.getScanner(scan), ScanType.USER_SCAN)) {
  ...
  public InternalScanner preFlush(ObserverContext<RegionCoprocessorEnvironment> c, Store store,
    ...
    byte[] family = store.getColumnFamilyDescriptor().getName();
